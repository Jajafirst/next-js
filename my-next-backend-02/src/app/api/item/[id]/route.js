...
export async function PATCH(req, { params }) {
  const { id } = await params;
  const data = await req.json(); // assume that it contains part of data...
  const partialUpdate = {};
  ...
  try {
    ...
    // Note: Usually in PATCH you only $set the partialUpdate, but keeping your logic here:
...
export async function PUT(req, { params }) {
  const { id } = await params;
  const data = await req.json(); // assume that it contains whole item data...
